    ...
    return (
        <div>
            {data && <div className="w-full flex flex-col items-center gap-5">
                ...
                <Divider/>
                <p className="leading-10">{language === "kh" ? data[0].khmerContent : data[0].content}</p>
                <div className="grid grid-cols-2 gap-5 phone-xl:grid-cols-1">
                    {data.map((service) => (
                        <div className="shadow-md mt-3 px-3 py-4 flex items-start gap-3 cursor-pointer shadow-normal duration-300 hover:shadow-3d-primary tablet-lg:flex-col phone-xl:items-center" key={service.id}>
